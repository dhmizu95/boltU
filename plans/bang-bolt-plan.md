# bang-bolt — Building an LLM From Scratch on Colab and Kaggle

Adapted from `guideline.md`, restructured for cloud notebooks (Colab CLI and Kaggle) instead of a local NVIDIA GPU.
...
colab new -s bang-bolt --gpu L4            # or T4 / A100 / H100
colab install -s bang-bolt -r requirements.txt
colab drivemount -s bang-bolt              # mounts Drive at /content/drive
echo "import torch; print(torch.cuda.get_device_name(0))" | colab exec -s bang-bolt
```
...
- [ ] Drive is mounted and `/content/drive/MyDrive/bang-bolt/` exists
...
```bash
# once
cp data/*.bin /content/drive/MyDrive/bang-bolt/data/
# every session thereafter
cp /content/drive/MyDrive/bang-bolt/data/*.bin /content/data/
```
...
colab exec -s bang-bolt -f src/train.py --config configs/tiny.yaml --max_steps 50

# Real run
colab exec -s bang-bolt -f src/train.py --config configs/base.yaml
```
...
1. `colab new -s bang-bolt --gpu L4` and `colab drivemount -s bang-bolt`
2. Copy data shards Drive → `/content/data/`
3. Copy `ckpt_latest.pt` Drive → `/content/checkpoints/`
4. `colab exec -s bang-bolt -f src/train.py --resume`
5. Let it run; the CLI keep-alive daemon handles idle timeouts
6. Confirm checkpoints are landing in Drive, then `colab stop -s bang-bolt`
...
**Optional local desktop GUI:** if you want the guideline's original Tkinter app, download the checkpoint (`colab download -s bang-bolt checkpoints/ckpt_best.pt ./`) and run it locally. A 124M model runs fine on CPU for inference — a few tokens/sec, adequate for testing.
...
while true; do
  colab download -s bang-bolt logs/metrics.csv /tmp/metrics.csv 2>/dev/null
  colab download -s bang-bolt logs/heartbeat.json /tmp/heartbeat.json 2>/dev/null
...
```json
{
  ...
  "dataset_sources": ["<username>/bang-bolt-tokens", "<username>/bang-bolt-checkpoints"]
}
```

```bash
kaggle kernels push -p ./kernel                    # submit
kaggle kernels status <username>/bang-bolt         # poll
kaggle kernels output <username>/bang-bolt -p ./out  # retrieve after completion
```
...
Next run attaches it and resumes from `/kaggle/input/bang-bolt-checkpoints/ckpt_latest.pt`. The §7 Drive-pruning script is unnecessary here — keep the version history.
...
- [ ] **P5c** *(Kaggle only)* `--max-hours` self-termination verified; token shards uploaded as a private dataset; checkpoint dataset versioning tested end-to-end; `enable_internet: true` confirmed
