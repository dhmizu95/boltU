...
```json
{
  ...
  "dataset_sources": ["<username>/llm-tokens", "<username>/llm-checkpoints"]
}
```

```bash
kaggle kernels push -p ./kernel                    # submit
kaggle kernels status <username>/llm-train         # poll
kaggle kernels output <username>/llm-train -p ./out  # retrieve after completion
```
...
Next run attaches it and resumes from `/kaggle/input/llm-checkpoints/ckpt_latest.pt`. The §7 Drive-pruning script is unnecessary here — keep the version history.
